...
export interface paths {
    ...
    "/v1/port-orders/{portOrder}/submit": {
         ...
         *     passing it starts real work against carriers — every porting carrier is asked, in the
         ...
         *     afterwards, and their answers are for our review desk, not part of this resource, so there is
         *     nothing further for you to wait for or poll. A carrier we cannot reach never fails this call.
         *
    ...
    "/v1/port-orders/{portOrder}/documents": {
         ...
         *     20 MB PDF, and no signed-URL handshake to perform. A `kind` field, a `file` part, and — on
         *     the `loa` slot — the two facts a scan cannot carry.
         *
         *     **A signed LOA is a signature, not only a file.** `kind=loa` REQUIRES `signed_on` (the day
         *     your customer put pen to paper, `YYYY-MM-DD`, never a day that has not happened) and `signer`
         *     (the name in the signature block). They are written in the same transaction as the document,
         *     so an order can never hold a signed letter whose signing date nobody recorded — which reads
         *     finished on every screen and is refused when you submit it. A refusal names the part in
         *     `source.parameter`. **This changed on 2026-09-03**: a `loa` upload with a file alone used to
         *     be accepted, and is now a **422**.
         *
         *     **A signed LOA needs a letter to be on.** Generate it first
         *     (`POST /v1/port-orders/{portOrder}/generate-loa`) — a scan of a letter this console never
         *     wrote can be compared against nothing. Uploading one before then is a **409**.
         *
         *     **The `bill` slot takes neither part**, and is unchanged in every other respect. Nobody signs
         *     a phone bill.
         *
         ...
         *
         *     **Only a draft takes an upload** — with one exception, and it is the paper one: a signed LOA
         *     is also accepted while the order is out for signature, because that is precisely how a wet
         *     signature happens. Every other upload stays draft-only: while your customer is signing, the
         *     paperwork has to keep matching the letter in front of them.
         *
    ...
    "/v1/pbx/subscribers": {
         ...
         * @description Every subscriber on your customers' phone systems, extension first — people AND machines.
         *     Read `kind` to tell them apart, or ask for the kinds you want with `filter[kind]`.
         *
         *     **For a click-to-call picker**, send `filter[kind]=user&filter[hasDevices]=true`: the people
         *     who have a phone. A subscriber with no device cannot place a call.
         *
         ...
         *     subscribers".
         */
        get: operations["listPbxSubscribers"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
    ...
    "/v1/pbx/subscribers/{subscriber}": {
         ...
         */
        get: operations["getPbxSubscriber"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
    ...
    "/v1/pbx/subscribers/{subscriber}/calls": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
         ...
         * @description Asks the phone system to ring `{subscriber}`'s phone and connect it to `destination`.
         *
         ...
         *
         *     A subscriber outside your customers' domains answers **404**, not 403 — the same answer an id that
    ...
    "/v1/pbx/call-records/{callRecord}/transcripts": {
        ...
        /**
         * List a call record's transcripts, each with a download link
         * @description What was said on a recorded call. One call can be captured more than once — the phone system
         ...
         */
        get: operations["listCallRecordTranscripts"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
...
export interface components {
    schemas: {
        ...
        /**
         * @description The parts an upload is made of. PDF only, decided on the file's own first five bytes, and at
         *     most 20 MB.
         *
        ...
        PortOrderDocumentUploadRequest: {
            ...
            /**
             * Format: date
             * @description **`loa` only, and required there.** The day your customer signed, as `YYYY-MM-DD`. Never
             *     a day that has not happened — an LOA dated after the day a carrier reads it authorizes
             *     nothing yet. Today is decided in UTC.
             * @example 2026-08-20
             */
            signed_on?: string;
            /**
        ...
        /**
         * @description A subscriber on a customer's phone system — a person or a machine; `kind` says which. Every
         *     field is read-only: this API does not write the phone system, and the write surface answers
         *     with a change INTENT rather than an update.
         */
        PbxSubscriberAttributes: {
            ...
            updatedAt?: string | null;
            kind?: components["schemas"]["PbxSubscriberKind"];
        };
        /**
         * @description What this subscriber is, derived on every read from the phone system's own markers. `user`
         *     is a person. `domain` is the domain's settings template, never a phone. `system` is any
         *     machine this API has no word for yet: an unknown marker is never read as `user`, so a
         *     `filter[kind]=user` list cannot gain a machine. New words may be added; read an unknown word
         *     as `system`.
         * @enum {string}
         */
        ...
        PbxSubscriberResource: {
             ...
             */
            id: string;
            attributes?: components["schemas"]["PbxSubscriberAttributes"];
            relationships?: components["schemas"]["PbxSubscriberRelationships"];
            links?: components["schemas"]["ResourceLinks"];
            meta?: components["schemas"]["ResourceMeta"];
        };
        PbxSubscriberDocumentResponse: {
            data: components["schemas"]["PbxSubscriberResource"];
            links?: components["schemas"]["ResourceLinks"];
            meta?: components["schemas"]["DocumentMeta"];
        };
        PbxSubscriberCollectionDocument: {
            data: components["schemas"]["PbxSubscriberResource"][];
            links?: components["schemas"]["CollectionLinks"];
            meta?: components["schemas"]["DocumentMeta"];
        };
        ...
        PbxDeviceRelationships: {
            customer?: components["schemas"]["RelationshipToOne"];
            subscriber?: components["schemas"]["RelationshipToOne"];
        };
        PbxDeviceResource: {
            /** @enum {string} */
            type: "devices";
        ...
        CallRecordRelationships: {
            customer?: components["schemas"]["RelationshipToOne"];
            fromSubscriber?: components["schemas"]["RelationshipToOne"];
            toSubscriber?: components["schemas"]["RelationshipToOne"];
        };
        CallRecordResource: {
            /** @enum {string} */
            type: "call-records";
        ...
        TranscriptSegment: {
            /**
             * @description Who was speaking, as a label rather than an identity: `Speaker 1`, `Speaker 2`, and so
             *     on. It comes from the speech-to-text provider's own separation of voices (or of the two
             *     sides of a stereo capture), so it is stable WITHIN one transcript and means nothing
             *     across two — `Speaker 1` on one call is not `Speaker 1` on another.
             * @example Speaker 1
             */
            speaker: string;
            /**
            ...
            /**
             * Format: double
             * @description When this turn ends, in seconds from the start of the recording.
             */
            end: number;
        ...
        TranscriptResource: {
            ...
            /**
             * Format: uuid
             * @description The RECORDING's id — a transcript is keyed by the capture it is of, one to one — so it is
             *     the same id the recordings list publishes and the same id in every region.
             */
            id: string;
    ...
    parameters: {
        ...
        /** @description The customer's id. A customer that is not on your account is a 404. */
        CustomerId: string;
        /** @description The PBX subscriber's id. */
        PbxSubscriberId: string;
...
export interface operations {
    ...
    uploadPortOrderDocument: {
        ...
        responses: {
            ...
            404: components["responses"]["NotFound"];
            /**
             * @description Two state conflicts, told apart by `title`. `Not editable` — the order is no longer a
             *     draft, so its paperwork is frozen, and `detail` names the state it is in (a `loa` upload
             *     is still accepted from an order out for signature). `No letter to sign` — the LOA has not
             *     been generated yet, so there is nothing a signed copy could be compared against.
             */
            409: {
                headers: {
                    [name: string]: unknown;
             ...
             *     PDF, or past the size ceiling. A `loa` upload missing `signed_on` or `signer`, or dated
             *     in the future, lands here too, and `source.parameter` names the part. The message names
    ...
    listPbxSubscribers: {
        ...
        requestBody?: never;
        responses: {
            /** @description A page of PBX subscribers. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                     ...
                     *     }
                     */
                    "application/vnd.api+json": components["schemas"]["PbxSubscriberCollectionDocument"];
                };
            };
            400: components["responses"]["BadQuery"];
            401: components["responses"]["Unauthenticated"];
    ...
    getPbxSubscriber: {
        parameters: {
            ...
            path: {
                /** @description The PBX subscriber's id. */
                subscriber: components["parameters"]["PbxSubscriberId"];
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description The PBX subscriber. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/vnd.api+json": components["schemas"]["PbxSubscriberDocumentResponse"];
                };
            };
            400: components["responses"]["BadQuery"];
            401: components["responses"]["Unauthenticated"];
    ...
    listCallRecordTranscripts: {
        ...
        responses: {
            /**
             * @description The call's transcripts, in vendor key order `(call_id, ccc_id)` — NOT chronological
             *     across legs. An EMPTY `data` array is a normal answer: a call that was not recorded has
             *     no captures to transcribe.
             */
            200: {
                headers: {
                    [name: string]: unknown;
                ...
                content: {
                     ...
                     *     }
                     */
                    "application/vnd.api+json": components["schemas"]["TranscriptCollectionDocument"];
                };
            };
            401: components["responses"]["Unauthenticated"];
            403: components["responses"]["Forbidden"];
    ...
    getCallRecordTranscript: {
        ...
        requestBody?: never;
        responses: {
            /** @description The transcript, with its turns. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                     ...
                     *     }
                     */
                    "application/vnd.api+json": components["schemas"]["TranscriptDocumentResponse"];
                };
            };
            401: components["responses"]["Unauthenticated"];
            403: components["responses"]["Forbidden"];
